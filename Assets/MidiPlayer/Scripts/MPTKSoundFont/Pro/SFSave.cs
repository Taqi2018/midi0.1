    ...
    public class SFSave
    {
        ...
        private int SaveInfo(int size)
        {
            ...
            {               /* loop over infos */
                ...
                if (SFFile.Verbose) SFFile.Log(SFFile.LogLevel.Info, "   INFO {0} '{1}' Conv. string to bytes: {2} --> {3} size:{4} file pos advance:{5}", info.id, info.Text, info.Text.Length, bytes.Length, x + 8, fd.BaseStream.Position - lastpos);
            }
            return size;
        }
        ...
        private int SaveSDta(int size)
        {
            ...
            if (SfData.Source == SFFile.SfSource.SF2)
            {
                fd.Write(SfData.SampleData);
                size += SfData.SampleData.Length + 8;         /* for sample chunk id (top of function) */
                ChunkSize(size);   /* update chunk size field */
            }
            else
            {
                // Remove sample data for MPTK, keep the chunk but empty
        ...
        private int SaveSHdr(int size)
        {
            ...
            foreach (HiSample sam in SfData.Samples)
            {
                //if (sam.Name.Contains("Grand-Piano-C4")) Debug.Break();

                zero_namestr(sam.Name);
                WriteStrSize(sam.Name, 20);

                if (SfData.Source == SFFile.SfSource.MPTK)
                {
                    //start always at 0 for MPTK. Sample data are removed when saving for MPTK. Keep only data pointer for sample extraction (to be removed in V3 ?)
                    fd.Write((uint)0); // start - start
                    //dummy = sam.End - sam.Start; end (lengh for MPTK)
                    fd.Write((uint)(sam.End - sam.Start));
                    //dummy = sam.LoopStart + sam.Start; Loop start
                    fd.Write((uint)(sam.LoopStart - sam.Start));
                    //dummy = sam.LoopEnd + sam.Start;
                    fd.Write((uint)(sam.LoopEnd - sam.Start)); // Loop end
                }
                else
                {
                    fd.Write((uint)sam.Start);
                    /* 
                      sample end, loopstart and loopend are offsets, SHDR uses absolute values, so add start. 
                      Comment TBN: disabled to be compliant with SF2 editor and merging SF (disabled also in FixupSample)
                      Sample end adds an additonal 1 because SHDR end should point to first point after sample, whereas we have it point to last sample.
                    */
                    //dummy = sam.End - sam.Start;
                    fd.Write((uint)(sam.End));// + sam.Start + 1));
                    //dummy = sam.LoopStart + sam.Start;
                    fd.Write((uint)(sam.LoopStart));// + sam.Start));
                    //dummy = sam.LoopEnd + sam.Start;
                    fd.Write((uint)(sam.LoopEnd));// + sam.Start));
                }
                fd.Write((uint)sam.SampleRate);
                fd.Write((sbyte)sam.OrigPitch);
                fd.Write((sbyte)sam.PitchAdj);
                fd.Write(dumzero);    /* put word 0 in sample link */
                fd.Write((ushort)sam.SampleType);
                size += SFFile.SFSHDRSIZE;
                //Debug.LogFormat("SaveSHdr {0} length:{1} loopstart:{2} loopend:{3} rate:{4} pitch:{5} pitchadj:{6} ",
                //    sam.Name,
                //    (uint)(sam.End - sam.Start), 
                //    (uint)(sam.LoopStart - sam.Start),
                //    (uint)(sam.LoopEnd - sam.Start),
                //    (uint)sam.SampleRate,
                //    (sbyte)sam.OrigPitch, 
                //    (sbyte)sam.PitchAdj);
            }
